...
/**
 * JSON-LD용 — 화면에 보이는 답변과 동일한 plain text
 */
export function formatHomeFaqAnswerPlainText(answer: HomeFaqAnswer): string {
  if (typeof answer === "string") return answer;

  const parts: string[] = [answer.intro, ...answer.bullets];
  if (answer.outro != null) parts.push(answer.outro);
  if (answer.tip != null) {
    parts.push(`${answer.tip.title}. ${answer.tip.body}`);
  }
  return parts.join(" ");
}
